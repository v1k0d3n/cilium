...
func readVersion(r io.Reader) int16 {
...
func ReadRequest(reader io.Reader) (*RequestMessage, error) {
	...
	buf := bytes.NewBuffer(req.rawMsg)

	req.version = readVersion(bytes.NewBuffer(req.rawMsg))
	...
	if err != nil {
		log.WithFields(log.Fields{
			"request": req.String(),
		}).WithError(err).Debug("Ignoring Kafka message parse error")
	}
