...
func (p *Proxy) CreateOrUpdateRedirect(l4 *policy.L4Filter, id string, source ProxySource, kind string) (Redirect, error) {
	...
	if r, ok := p.redirects[id]; ok {
		...
		scopedLog.WithField("obj", logfields.PrintStruct(r)).Debug("updated existing proxy instance")
		return r, nil
	}
	...
	log.WithField("obj", logfields.PrintStruct(redir)).Debug("Created new proxy instance")
