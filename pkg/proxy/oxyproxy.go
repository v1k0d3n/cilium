...
func lookupNewDest(req *http.Request, dport uint16) (uint32, string, error) {
	...
	if pIP.To4() != nil {
		...
		// REVIEW Should this use a field?
...
func createOxyRedirect(l4 *policy.L4Filter, id string, source ProxySource, to uint16) (Redirect, error) {
	...
	redirect := http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			...
			log.WithError(err).Errorf("cannot generate url")
		...
		if len(redir.rules) > 0 {
			...
			ar := rule.(string)
			log.WithField("rule", logfields.PrintStruct(ar)).Debug("Allowing request based on rule")
