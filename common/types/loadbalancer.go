...
func (lb *LoadBalancer) AddService(svc LBSVC) bool {
	...
	if ok {
		...
		}).Debug("service is already in lb.SVCMapID; deleting old entry and updating it with new entry")
	...
	log.WithFields(log.Fields{
		logfields.ServiceName: svc.FE.String(),
		logfields.ServiceSHA:  svc.Sha256,
	}).Debug("adding service to lb.SVCMap")
	lb.SVCMap[svc.Sha256] = svc

	log.WithFields(log.Fields{
		logfields.ServiceName: svc.FE.String(),
		logfields.ServiceSHA:  svc.Sha256,
	}).Debug("adding service to lb.SVCMapID")
...
func (lb *LoadBalancer) DeleteService(svc *LBSVC) {
	log.WithFields(log.Fields{
		logfields.ServiceName: svc.FE.String(),
		logfields.ServiceSHA:  svc.Sha256,
	}).Debug("deleting service from lb.SVCMap, lb.SVCMapID")
