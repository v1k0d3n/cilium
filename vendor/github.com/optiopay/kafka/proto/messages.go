...
func (w *slicewriter) Reset(size int) {
	if size > len(w.buf) {
		w.buf = allocParseBuf(size + 1000) // allocate a bit more than required
	}
	w.size = size
	w.pos = 0
}
...
func readMessageSet(r io.Reader, size int32) ([]*Message, error) {
	if size < 0 {
		return nil, fmt.Errorf("negative messageSet size: %d", size)
	}

	rd := io.LimitReader(r, int64(size))

	if !ReadMessages {
		msgbuf := allocParseBuf(int(size))
