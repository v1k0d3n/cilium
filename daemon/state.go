...
func (d *Daemon) SyncState(dir string, clean bool) error {
	...
	for _, ep := range possibleEPs {
		...
		}).Debug("Restoring endpoint")

		if err := d.syncLabels(ep); err != nil {
			log.WithError(err).WithFields(log.Fields{
				logfields.EndpointID: ep.ID,
			}).Warning("Unable to restore endpoint")
			continue
		}
	...
	for k := range endpointmanager.Endpoints {
		...
		go func(ep *endpoint.Endpoint, wg chan<- bool) {
			if err := d.allocateIPs(ep); err != nil {
				log.WithError(err).WithFields(log.Fields{
					logfields.EndpointID: ep.ID,
				}).Error("Failed to re-allocate IP of endpoint. Not restoring endpoint.")
				d.deleteEndpoint(ep)
				wg <- false
				return
			}
			...
			if buildSuccess := <-ep.Regenerate(d); !buildSuccess {
				log.WithError(err).WithFields(log.Fields{
					logfields.EndpointID: ep.ID,
				}).Warning("Failed while regenerating endpoint")
				wg <- false
				return
			}
			ep.Mutex.RLock()
			log.WithError(err).WithFields(log.Fields{
				logfields.EndpointID: ep.ID,
				logfields.IPAddr:     []string{ep.IPv4.String(), ep.IPv6.String()},
			}).Info("Restored endpoint with IPs")
			ep.Mutex.RUnlock()
			wg <- true
		}(ep, wg)
	}
	...
	for _, epID := range eptsDirNames {
		...
		readDir := func() string {
			...
			if err != nil {
				log.WithFields(log.Fields{
					logfields.Path: epDir,
				}).Warning("Error while reading directory. Ignoring it...")
				return ""
			}
			cHeaderFile := common.FindEPConfigCHeader(epDir, epFiles)
			if cHeaderFile == "" {
				log.WithFields(log.Fields{
					logfields.Path:       []string{epDir, common.CHeaderFileName},
					logfields.EndpointID: epID,
				}).Info("File not found. Ignoring endpoint.")
				return ""
			}
			return cHeaderFile
		}
		...
		}).Debugf("Found endpoint C header file")

		strEp, err := common.GetCiliumVersionString(cHeaderFile)
		if err != nil {
			log.WithError(err).WithFields(log.Fields{
				logfields.Path: cHeaderFile,
			}).Warning("Unable to read the C header file")
			continue
		}
