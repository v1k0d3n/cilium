...
func (d *Daemon) addSVC2BPFMap(feCilium types.L3n4AddrID, feBPF lbmap.ServiceKey,
	...
	log.WithFields(log.Fields{
		logfields.ServiceName: feCilium.String(),
	}).Debug("adding service to BPF maps")
	...
	if err != nil {
		log.WithError(err).WithFields(log.Fields{
			logfields.ServiceName: feCilium.L3n4Addr.String(),
		}).Debug("error deleting service before adding it")
	}
	...
	if addRevNAT {
		log.WithFields(log.Fields{
			logfields.ServiceName: feCilium.String(),
		}).Debug("adding service to RevNATMap")
...
func (h *deleteServiceID) Handle(params DeleteServiceIDParams) middleware.Responder {
	log.WithFields(log.Fields{
		"params": params,
	}).Debug("DELETE /service/{id} request")
	...
	if err := h.d.svcDelete(svc); err != nil {
		log.WithError(err).WithFields(log.Fields{
			logfields.ServiceID:  svc.FE,
			logfields.ServiceSHA: svc.Sha256,
		}).Warning("DELETE /service/{id}: error deleting service")
...
func (d *Daemon) svcDeleteBPF(svc *types.LBSVC) error {
	...
	}).Debug("deleting service from BPF maps")
	...
	}).Debug("done deleting service slaves, now deleting master service")
...
func (h *getServiceID) Handle(params GetServiceIDParams) middleware.Responder {
	log.WithFields(log.Fields{
		"params": params,
	}).Debug("GET /service/{id} request: %+v")
...
func (d *Daemon) SyncLBMap() error {
	...
	addSVC2BPFMap := func(oldID types.ServiceID, svc types.LBSVC) error {
		...
		funcLog.Debug("adding service ID with SHA")
		...
		if ok {
			funcLog.Debug("Service ID is present in BPF map, updating revnat key")
			...
			if err := lbmap.DeleteRevNat(revNATK); err != nil {
				funcLog.WithError(err).Warning("Unable to remove old rev NAT entry")
			}

			funcLog.Debug("deleting old ID from newRevNATMap")
	...
	parseSVCEntries := func(key bpf.MapKey, value bpf.MapValue) {
		...
		funcLog := log.WithFields(log.Fields{
			logfields.BPFServiceKey: svcKey,
			logfields.BPFService:    svcValue,
		})

		funcLog.Debug("parsing service mapping")
		fe, be, err := lbmap.ServiceKeynValue2FEnBE(svcKey, svcValue)
		if err != nil {
			funcLog.WithError(err).Errorf("SyncLBMap.parseSVCEntries")
			return
		}
	...
	parseRevNATEntries := func(key bpf.MapKey, value bpf.MapValue) {
		...
		funcLog := log.WithFields(log.Fields{
			logfields.BPFServiceKey: revNatK,
			logfields.BPFService:    revNatV,
		})

		funcLog.Debug("parsing BPF revNAT mapping")
		fe, err := lbmap.RevNatValue2L3n4AddrID(revNatK, revNatV)
		if err != nil {
			funcLog.WithError(err).Error("SyncLBMap.parseRevNATEntries")
			return
		}
	...
	for _, svc := range newSVCList {
		...
		if err != nil {
			log.WithError(err).WithFields(log.Fields{
				logfields.ServiceID: svc.FE.ID,
				logfields.IPAddr:    svc.FE.L3n4Addr.String(),
		...
		if svc.FE.ID != kvL3n4AddrID.ID {
			...
			}).Infof("service ID read from BPF map was out of sync with KVStore, got new ID")
			oldID := svc.FE.ID
	...
	for _, svc := range failedSyncSVC {
		log.WithFields(log.Fields{
			logfields.Service: svc.FE,
		}).Debug("Unable to restore, so removing service")
		if err := d.svcDeleteBPF(&svc); err != nil {
			log.WithError(err).WithFields(log.Fields{
				logfields.Service: svc.FE,
			}).Warning("Unable to clean service from BPF map")
		}
	}
